...
/**
 * A SwitchICS is a two-state toggle switch widget that can select between two
 ...
 * 
 */
public class SwitchICS extends CompoundButton
{
	 ...
	 */
	public SwitchICS(Context context)
	{
		this(context, null);
	}
	 ...
	 *            styling.
	 */
	public SwitchICS(Context context, AttributeSet attrs)
	{
		this(context, attrs, R.attr.switchICSStyleAttr);
	}
	 ...
	 */
	public SwitchICS(Context context, AttributeSet attrs, int defStyle)
	{
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SwitchICS, defStyle, 0);

		mThumbDrawable = a.getDrawable(R.styleable.SwitchICS_thumb);
		mTrackDrawable = a.getDrawable(R.styleable.SwitchICS_track);
		mTextOn = a.getText(R.styleable.SwitchICS_textOn);
		mTextOff = a.getText(R.styleable.SwitchICS_textOff);
		mTextOutsideTrack = a.getBoolean(R.styleable.SwitchICS_textOutsideTrack, false);
		mTextOnThumb = a.getBoolean(R.styleable.SwitchICS_textOnThumb, false);
		mThumbTextPadding = a.getDimensionPixelSize(R.styleable.SwitchICS_thumbTextPadding, 0);
		mTrackTextPadding = a.getDimensionPixelSize(R.styleable.SwitchICS_trackTextPadding, 0);
		mSwitchMinWidth = a.getDimensionPixelSize(R.styleable.SwitchICS_switchMinWidth, 0);
		mSwitchMinHeight = a.getDimensionPixelSize(R.styleable.SwitchICS_switchMinHeight, 0);
		mSwitchPadding = a.getDimensionPixelSize(R.styleable.SwitchICS_switchPadding, 0);
		...
		int appearance = a.getResourceId(R.styleable.SwitchICS_switchTextAppearanceAttrib, 0);
		if (appearance != 0)
		{
	...
	{
		TypedArray appearance = context.obtainStyledAttributes(resid, R.styleable.switchICSTextAppearanceAttrib);

		ColorStateList colors;
		int ts;

		colors = appearance.getColorStateList(R.styleable.switchICSTextAppearanceAttrib_textColor);
		if (colors != null)
		{
			mTextColors = colors;
		} else
		...
		ts = appearance.getDimensionPixelSize(R.styleable.switchICSTextAppearanceAttrib_textSize, 0);
		if (ts != 0)
		{
		...
		typefaceIndex = appearance.getInt(R.styleable.switchICSTextAppearanceAttrib_my_switch_typeface, -1);
		styleIndex = appearance.getInt(R.styleable.switchICSTextAppearanceAttrib_textStyle, -1);
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		if (mTextOnThumb == false)
		{
			...
			if (mThumbWidth < 15)
			{
				// TODO: change this to something gueessed based on the other
				// parameters.
				mThumbWidth = 15;
			}
		}
		...
		// Log.d(TAG,
		// "mSwitchWidth="+mSwitchWidth+" mSwitchHeight="+mSwitchHeight);
	...
	{
		// LoggingUtility.d(TAG, "onLayout()-left=" + left +
	...
	protected void onDraw(Canvas canvas)
	{
		...
		int dxOffText = mTextOnThumb ? (thumbL + thumbR) / 2 - mOffLayout.getWidth() / 2 - mThumbTextPadding // (thumbL+thumbR)/2
																												// already
																												// has
																												// 2*mThumbTextPadding
																												// so
																												...
																												// subtract
																												// it
				+ mTrackTextPadding : switchInnerLeft + mTrackTextPadding;

		int thumbPos = getThumbScrollRange();
		thumbL = thumbL + thumbPos;
		thumbR = thumbR + thumbPos;
		int dxOnText = mTextOnThumb ? (thumbL + thumbR) / 2 // (thumbL +
															// thumbR)/2 already
															// has the
															// ThumbTextPadding
															// so we dont have
															// to add it
				- mOnLayout.getWidth() / 2 : switchInnerRight - mOnLayout.getWidth() - mTrackTextPadding;
