 ...
 * @author Maria Dzyokh
 */
public class RisingStarsFragment extends AbstractFragment {

    public static RisingStarsFragment newInstance() {
        return new RisingStarsFragment();
    }
    ...
    public void loadData(int page, String nextPageToken, boolean showProgress) {
        ...
        if (lastKnownLocation != null) {
            ...
            float filterDistance = Float.parseFloat(getApp().getMainSharedPrefs().getString(Settings.RISING_STARS_FILTER_DISTANCE, "0"));
            int category = getApp().getMainSharedPrefs().getInt(Settings.RISING_STARS_FILTER_CATEGORY, 0);
            VibeItApiUtils.getRisingStars(getActivity(), locationsReceiver, userToken, lastKnownLocation.getLatitude(), lastKnownLocation.getLongitude(), filterDistance, category, getSearchArgs(), page, nextPageToken);
        } else {
//            if (aq!=null) {
...
//            }
    ...
    public View getMenu() {
        ...
        if (getActivity()!=null) {
            ...
            ((TextView)menuHeader.findViewById(R.id.filter_title)).setText("Rising Stars Filter");
            VibeFilter distanceFilter = (VibeFilter) menuHeader.findViewById(R.id.distance_filter);
            distanceFilter.setCurrentValue(getApp().getMainSharedPrefs().getString(Settings.RISING_STARS_FILTER_DISTANCE, "0"));
            distanceFilter.setValueChangedListener(new VibeFilter.ValueChangedListener() {
                @Override
                public void onValueChanged(String newValue) {
                    getApp().getMainSharedPrefs().edit().putString(Settings.RISING_STARS_FILTER_DISTANCE, newValue).commit();
                    prefsChanged = true;
                }
            });
            ...
            categoriesList.setItemChecked(getApp().getMainSharedPrefs().getInt(Settings.RISING_STARS_FILTER_CATEGORY, 0) + 1, true);
            categoriesList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                    getApp().getMainSharedPrefs().edit().putInt(Settings.RISING_STARS_FILTER_CATEGORY, (int) l).commit();
                    prefsChanged = true;
